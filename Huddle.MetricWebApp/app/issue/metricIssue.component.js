...
var MetricIssueComponent = /** @class */ (function () {
    ...
    MetricIssueComponent.prototype.getMetricValues = function () {
        ...
        this.issueWeekInputviewModel.metricValueArray.forEach(function (metricValue) {
            metricValue.metricValues = null;
            //changed to metric
            //metricValue.issue = this.currentIssue;
            metricValue.id = 0;
        });
        this.reasonWeekInputViewModelArray.forEach(function (reasonWeekVM) { return reasonWeekVM.reasonValueArray.forEach(function (reasonMetric) {
        ...
        this.metricValueService.getMetricValues(this.currentIssue.id, this.issueReasons.map(function (reason) { return reason.id; }), this.selectWeekDay)
            ...
            resp.forEach(function (weekInputWM) {
                if (weekInputWM.isMetricValue) {
                    _this.issueWeekInputviewModel.metricValueArray.forEach(function (metricValue, index) {
                        var backendIssueMetric = weekInputWM.metricValueArray.find(function (im) { return self.isDateEqual(metricValue.inputDate, im.inputDate); });
                        if (backendIssueMetric) {
                            metricValue.metricValues = backendIssueMetric.metricValues;
                            metricValue.id = backendIssueMetric.id;
                            metricValue.inputDate = dateHelper_1.DateHelper.UTCToLocal(metricValue.inputDate);
                        }
                    });
                }
                else {
                    if (weekInputWM.reasonValueArray.length > 0) {
                        var targetReasonWeekInputVM = _this.reasonWeekInputViewModelArray.find(function (reasonWeekInputVm) { return reasonWeekInputVm.reasonValueArray[0].reason.id == weekInputWM.reasonValueArray[0].reason.id; });
                        if (targetReasonWeekInputVM) {
                            targetReasonWeekInputVM.reasonValueArray.forEach(function (reasonMetric, index) {
                                var backendReasonMetric = weekInputWM.reasonValueArray.find(function (rm) { return self.isDateEqual(reasonMetric.inputDate, rm.inputDate); });
    ...
    MetricIssueComponent.prototype.checkData = function () {
        ...
        this.issueWeekInputviewModel.metricValueArray.forEach(function (metricValue) {
            if (_this.checkAndResetNaN(metricValue))
                checkResult = false;
        });
        this.reasonWeekInputViewModelArray.forEach(function (reasonWeekInputViewModel) {
            reasonWeekInputViewModel.reasonValueArray.forEach(function (reasonMetric) {
    ...
    MetricIssueComponent.prototype.saveData = function () {
        ...
        return this.metricValueService.updateMetricValues(this.issueWeekInputviewModel.metricValueArray, this.reasonWeekInputViewModelArray.map(function (reasonWeekVM) { return reasonWeekVM.reasonValueArray; }), this.currentMetricValues);
    };
    ...
    MetricIssueComponent = __decorate([
        ...
        __metadata("design:paramtypes", [typeof (_a = typeof router_1.ActivatedRoute !== "undefined" && router_1.ActivatedRoute) === "function" && _a || Object, issue_service_1.IssueService,
