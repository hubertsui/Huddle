...
var IssueService = /** @class */ (function () {
    ...
    IssueService.prototype.getUsers = function (teamId) {
        var _this = this;
        var activeObject = new Rx_1.ReplaySubject(1);
        this.dataService.getArray(constants_1.Constants.webAPI.teamsUrl + '/' + teamId)
            .subscribe(function (resp) {
            var result = [];
    ...
    IssueService.prototype.addIssue = function (issue, teamId) {
        var activeObject = new Rx_1.ReplaySubject(1);
        this.dataService.post(constants_1.Constants.webAPI.issuesUrl, { issue: modelConverter_1.ModelConverter.ToIssueBackend(issue), teamId: teamId })
            .subscribe(function (resp) {
            activeObject.next(resp.issueId);
        }, function (error) { return activeObject.error(error); });
        return activeObject;
    };
    IssueService.prototype.editIssue = function (issue) {
        var activeObject = new Rx_1.ReplaySubject(1);
        this.dataService.post(constants_1.Constants.webAPI.issueEditUrl, { issue: modelConverter_1.ModelConverter.ToIssueBackend(issue) })
            .subscribe(function (resp) {
            activeObject.next(resp.issueId);
        }, function (error) { return activeObject.error(error); });
        return activeObject;
